...
public class AdminService : IBootstrapAdminService
{
    ...
    {
        var ret = Navigations.GetAllMenus(userName).Any(m => m.Url.Contains(url, StringComparison.OrdinalIgnoreCase) && m.IsEnable);
    ...
    public bool AuthorizingBlock(string userName, string url, string blockName)
    {
        ...
        if (!ret)
        {
            ...
            var menu = menus.FirstOrDefault(m => m.Url.Contains(url, StringComparison.OrdinalIgnoreCase) && m.IsEnable);
            if (menu != null)
            {
