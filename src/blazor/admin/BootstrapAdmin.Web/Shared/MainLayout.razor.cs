    ...
    public partial class MainLayout : IDisposable
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            if (!string.IsNullOrEmpty(UserName))
            {
                ...
                Context.DisplayName = user?.DisplayName ?? Localizer["NoRegisterAccount"];
                Context.GroupId = user?.GroupsID;
                Context.VendorId = user?.VendorID;
                Icon = string.IsNullOrEmpty(user?.Icon) ? "/images/uploader/default.jpg" : GetIcon(user.Icon);

                MenuItems = NavigationsService.GetAllMenus(UserName).Where(menu => menu.IsEnable).ToMenus();
            }
        ...
        {
            MenuItems = NavigationsService.GetAllMenus(Context.UserName).Where(menu => menu.IsEnable).ToMenus();
            StateHasChanged();
            return Task.CompletedTask;
        }
