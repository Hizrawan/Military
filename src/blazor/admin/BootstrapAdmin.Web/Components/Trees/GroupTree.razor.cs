...
public partial class GroupTree
{
    [Inject]
    [NotNull]
    private IStringLocalizer<GroupTree>? Localizer { get; set; }
    ...
    private async Task OnSelectMenu()
    {
        ...
        {
            IsScrolling = true,
            Title = Localizer["GroupTreeTitle"],
            BodyTemplate = BootstrapDynamicComponent.CreateComponent<ParentGroupTree>(new Dictionary<string, object?>
            {
                [nameof(ParentGroupTree.Value)] = Value,
                [nameof(ParentGroupTree.ValueChanged)] = EventCallback.Factory.Create<string>(this, v => OnValueChanged(v))
            }).Render(),
            FooterTemplate = BootstrapDynamicComponent.CreateComponent<Button>(new Dictionary<string, object?>
            {
                ...
                [nameof(Button.Text)] = Localizer["ConfirmText"].ToString(),
