...
public partial class CardPhoto<TValue> : ButtonUploadBase<TValue>
{
    private string? BodyClassString => CssBuilder.Default("upload-body is-card")
        .AddClass("is-single", IsSingle)
        .Build();

    private string? GetDisabledString(UploadFile item) => (!IsDisabled &&  item.Code == 0) ? null : "disabled";  // item!.Uploaded  -> 這邊只負責刪除，因此為true

    private bool ShowPreviewList => GetUploadFiles().Any();

    private List<string?> PreviewList => GetUploadFiles().Select(i => i.PrevUrl).ToList();

    private string? GetDeleteButtonDisabledString(UploadFile item) => (!IsDisabled ) ? null : "disabled";  //&& item.Uploaded
    ...
    /// <summary>
    /// 因使用原來的UploadFile, 但沒有欄位可加載 FileDesc, 這里用圖片位址來比對取得圖片說明
    /// </summary>
    /// <param name="PreUrl"></param>
    /// <returns></returns>
    private string? GetFileDescByFilePath(string? PreUrl)
    {
        if (string.IsNullOrEmpty(PreUrl)) return null;

        return RepairFileService.GetFileDescByFilePath(PreUrl);
    }
}

/// <summary>
/// 繼承原 BootStrapBlazor.UploadFile, 多加一個欄位: 文件說明
/// </summary>
public class SDRUploadFile : UploadFile
{
    /// <summary>
    /// 照片說明
    /// </summary>
